  ...
  return (
    <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-3xl p-5 border-2 border-amber-200">
      <div className="flex items-center gap-2 mb-4">
        <Icon name="emoji_events" className="w-6 h-6 text-amber-600" />
      ...
      <div className="space-y-3">
        {topThree.map((province, index) => (
          <motion.div
            ...
            className={`bg-white rounded-2xl p-4 flex items-center gap-3 ${index === 0 ? 'ring-2 ring-amber-500 shadow-lg' : 'shadow-sm'
              }`}
          >
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${index === 0 ? 'bg-gradient-to-br from-amber-400 to-orange-500' :
                index === 1 ? 'bg-gradient-to-br from-slate-300 to-slate-400' :
                  'bg-gradient-to-br from-amber-600 to-amber-700'
              }`}>
              {index === 0 ? (
                <Icon name="social_leaderboard" className="w-6 h-6 text-white" />
              ) : (
