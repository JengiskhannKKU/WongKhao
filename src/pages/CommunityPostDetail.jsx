...
const mockPosts = [
    {
        ...
        saves: 683,
        commentsCount: 4,
        content: "อันนี้วิธีที่เราลดน้ำหนักนะคะ ทุกคนที่เตรียมตัวลดสามารถนำไปปรับใช้ได้เลยน้าา สู้ๆนะคะ ✌🏻✌🏻 ใครจะเริ่มลด หรือเตรียมลดทักมาคุยกันปรึกษากันได้เลยน้าา\n\nสำหรับพิกัดของกินสามารถเข้าไปดูในลิ้งหน้าไบโอน้าา\n\n#ลดน้ำหนัก #เมนูคลีน #อาหารคลีน #healthy",
        updatedAt: "1/10/2025",
        comments: [
            { id: 1, user: 'Jia Jia', avatar: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=50&h=50&fit=crop', text: 'ดีมากกกก ขอบคุณที่แชร์แนวทางความรู้นะคะ 🥰', date: '7/12/2025' },
            { id: 2, user: 'kk', avatar: 'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?w=50&h=50&fit=crop', text: '🥰', date: '8/9/2025' },
            { id: 3, user: 'Kero(コップ) 94 🍰', avatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=50&h=50&fit=crop', text: 'ขอบคุณมากค่ะ', date: '15/10/2025' },
            { id: 4, user: 'PRAISE', avatar: 'https://images.unsplash.com/photo-1517841905240-472988babdf9?w=50&h=50&fit=crop', text: 'เป็นประโยชน์มากๆค่ะ', date: '8/9/2025' }
        ]
    },
    {
        id: 2,
        ...
        saves: 300,
        commentsCount: 2,
        content: "ใครที่เบื่ออกไก่แห้งๆ ลองวิธีนี้ดูนะคะ รับรองว่าอกไก่นุ่มฉ่ำ ทานง่ายขึ้นเยอะเลย!\n\nส่วนผสม:\n- อกไก่ 500 กรัม\n- ซีอิ๊วขาวลดโซเดียม 1 ช้อนโต๊ะ\n- น้ำมันงา 1 ช้อนชา\n- เบกกิ้งโซดา หยิบมือ",
        updatedAt: "5/10/2025",
        comments: [
            { id: 1, user: 'Ploy', avatar: 'https://images.unsplash.com/photo-1524504388940-b1c1722653e1?w=50&h=50&fit=crop', text: 'ต้องลองทำตามแล้วค่ะ 😋', date: '6/10/2025' },
            { id: 2, user: 'Toey', avatar: 'https://images.unsplash.com/photo-1531427186611-ecfd6d936c79?w=50&h=50&fit=crop', text: 'ใช้หม้อทอดไร้น้ำมันได้มั้ยคะ', date: '6/10/2025' }
        ]
    },
    // Default fallback for any other ID
    {
        ...
        images: ['https://images.unsplash.com/photo-1559314809-0d155014e29e?w=800&q=80'],
        user: 'Egg Lover',
        avatar: 'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?w=100&h=100&fit=crop&q=80',
        likes: 2400,
        saves: 500,
        commentsCount: 1,
        content: "เมนูไข่ทำได้หลากหลาย ลองเปลี่ยนวัตถุดิบเล็กน้อยก็อร่อยไม่ซ้ำแล้วค่ะ\n\n#เมนูไข่ #ลดน้ำหนัก",
        updatedAt: "10/10/2025",
        comments: [
            { id: 1, user: 'Test User', avatar: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=50&h=50&fit=crop', text: 'น่าทานมากค่ะ', date: '11/10/2025' }
        ]
    }
];
...
export default function CommunityPostDetail() {
    ...
    useEffect(() => {
        ...
        const foundPost = mockPosts.find(p => p.id.toString() === id) || mockPosts.find(p => p.id === 'default');
        setPost(foundPost);
    }, [id]);
        ...
        <div className="min-h-screen bg-white flex flex-col pt-safe pb-safe">
            ...
            <div className="sticky top-0 z-50 bg-white/90 backdrop-blur-md px-4 py-3 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <button onClick={() => navigate(-1)} className="w-[36px] h-[36px] rounded-full flex items-center justify-center hover:bg-slate-100 transition-colors">
                        <Icon name="chevron_left" className="w-[26px] h-[26px] text-slate-800" />
                    </button>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium text-slate-800 text-[15px]">{post.user}</span>
                    </div>
                </div>
                        ...
                        <div key={comment.id} className="flex gap-3">
                            ...
                            <div className="flex-1">
                                <div className="flex items-center justify-between mb-0.5">
                                    <span className="text-[13px] font-medium text-slate-500">{comment.user}</span>
                                    <button className="text-slate-400">
                                        <Icon name="favorite_border" className="w-[18px] h-[18px]" />
                                    </button>
                                </div>
                                ...
                                <div className="flex items-center gap-3">
                                    <span className="text-[12px] text-slate-400">{comment.date}</span>
                                    <button className="text-[12px] font-bold text-slate-600">ตอบกลับ</button>
                                </div>
                            </div>
                        </div>
                    ))}
            ...
            <div className="sticky bottom-0 bg-white border-t border-slate-100 px-4 py-2.5 pb-safe-bottom">
                ...
                <div className="flex items-center justify-between pb-1">
                    <button className="flex items-center gap-2 text-slate-800">
                        ...
                        <span className="text-[13px] font-bold">{post.saves} คนบันทึกแล้ว</span>
                    </button>

                    <div className="flex items-center gap-5">
                        <button className="flex items-center gap-1.5 text-slate-800">
                            <Icon name="favorite_border" className="w-[22px] h-[22px]" />
                            <span className="text-[13px] font-medium opacity-90">{post.likes}</span>
                        </button>
                        <button className="flex items-center gap-1.5 text-slate-800">
                            <Icon name="chat_bubble_outline" className="w-[20px] h-[20px]" />
                            <span className="text-[13px] font-medium opacity-90">{post.commentsCount}</span>
                        </button>
